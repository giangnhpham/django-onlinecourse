...
def submit(request, course_id):
    ...
    # Then get the associate enrollement object
    enrollment = Enrollment.objects.get(user=user, course=course)

    # Create a new submission object referring to the enrollment
    submission = Submission.objects.create(enrollment=enrollment)

    # Collect the selected choice from the HTTP request (exam form)
...
def show_exam_result(request, course_id, submission_id):
    # Get course and submssion based on their ids
    course = get_object_or_404(Course, pk=course_id)
    ...
    context = {
        ...
        'grade': round((mark / total_mark) * 100)
    }
